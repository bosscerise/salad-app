...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const registerMutation = useMutation({
    mutationFn: async (data: RegisterUserData) => {
      const formData = {
        ...
        name: data.name,
        phone: data.phone || '', // Include phone in the form data
        role: data.role || 'customer',
        emailVisibility: false,  // Change this to false for better privacy
  ...
  // OAuth2 login function - FIXED to preserve admin role
  const loginWithOAuth2 = async (provider: string) => {
    setIsLoading(true);
    try {
      // Use the popup method for OAuth2 authentication with query params to force account selection
      ...
      console.log('OAuth2 login successful, user data:', { 
        id: authData.record.id,
        email: authData.record.email,
        role: authData.record.role 
      });
      
      // Don't override user role if they are already an admin
      // Only update if needed for non-admin users
      if (authData.record.role !== 'admin' && authData.record.emailVisibility !== false) {
        await pb.collection('users').update(authData.record.id, {
          // IMPORTANT: Don't change role, only update emailVisibility
          emailVisibility: false
        });
        console.log('Updated user emailVisibility while preserving role:', authData.record.role);
      }
      
      setIsLoading(false);
      await loadUserData(); // This will refresh the user data with correct role
      queryClient.invalidateQueries({ queryKey: ['user'] });
      ...
      const authResponse: AuthResponse = {
        ...
        record: {
          ...
          role: authData.record.role, // Use the actual role from the server
          emailVisibility: 'true',
