    ...
    >
      ...
      {showDetails && (
        <div className="mt-2 text-xs text-gray-500 sm:text-sm dark:text-gray-400">
          <div>Calories: {ingredient.calories || 0}</div>
          <div>Protein: {ingredient.protein || 0}g</div>
          <div>Carbs: {ingredient.carbs || 0}g</div>
          <div>Fat: {ingredient.fats || 0}</div>
        </div>
      )}
      {isSelected && (
...
export default function SaladBuilder() {
  ...
  const [savedSalads, setSavedSalads] = useState<UserSaladRecord[]>([]);
    ...
    const result = Object.entries(selectedIngredients).reduce(
      (acc, [id, count]) => {
        ...
        if (ingredient && count > 0) {
          ...
          acc.calories += (ingredient.calories || 0) * count;
  ...
  const handleAddToCart = useCallback(async () => {
    try {
      ...
      if (pb.authStore.isValid) {
        ...
        const savedSalad = await saladService.create(saladData);
  ...
  }, [selectedIngredients, saladName, totals, quantity, addToCart, toast]);
  ...
  const handleSaveSalad = useCallback(async () => {
    ...
    try {
      ...
      // Navigate to the saved salad detail page
      navigate(`/my-salads/${savedSalad.id}`);
    } catch (error) {
  ...
  }, [saladName, savedSalads.length, selectedIngredients, totals, navigate, toast]);
