...
import { pb } from '../../pb';
import { SaladRecord, IngredientRecord } from '../../pb/types';
...
interface ExtendedSaladRecord extends SaladRecord {
...
export default function SaladDetail() {
  ...
  const [salad, setSalad] = useState<ExtendedSaladRecord | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [quantity, setQuantity] = useState(1);
  const [relatedSalads, setRelatedSalads] = useState<SaladRecord[]>([]);
  ...
  const getImageURL = async (record: SaladRecord, image: string): Promise<string> => {
    try {
  ...
  const fetchSaladData = useCallback(async () => {
    ...
    try {
      ...
      let saladData: ExtendedSaladRecord | null = null;
      let relatedItems: SaladRecord[] = [];
      ...
      try {
        ...
        if (isNumericId) {
          ...
          if (fallbackSalad) {
            ...
            let additionalSalads: SaladRecord[] = [];
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="flex flex-col p-6 sm:p-8 lg:w-1/2">
              ...
              <div className="grid grid-cols-3 gap-4 mb-8">
                ...
                >
                  <span className="text-sm font-medium text-gray-700">{salad.prepTime || 5} min</span>
                </motion.div>
              </div>
              ...
              <div className="mt-auto">
                ...
                <motion.button
                  ...
                  whileTap={{ scale: 0.98 }}
                  onClick={() => {
                    if (isCustomizing && isCustomized && salad) {
                      try {
                        // Convert customized ingredients to proper format for cart
                        const customizationsRecord: Record<string, number> = {};
                        
                        customizedIngredients.forEach(ingredient => {
                          // Ensure we only include valid ingredients with IDs
                          if (ingredient.id) {
                            customizationsRecord[ingredient.id] = ingredient.quantity;
                          }
                        });
                        
                        // Only proceed if we have valid customizations
                        if (Object.keys(customizationsRecord).length > 0) {
                          addToCart({
                            id: `${salad.id}_custom_${Date.now()}`, // Add timestamp to make ID unique
                          ...
                          alert(`${quantity} × Custom ${salad.name} added to your cart!`);
                        } else {
                          alert('Please add at least one ingredient to customize your salad.');
                        }
                      } catch (error) {
                        console.error('Error adding customized salad to cart:', error);
                        alert('Sorry, there was a problem adding this item to your cart. Please try again.');
                      }
                    } else {
                      handleAddToCart();
                ...
                >
                  ...
                  <div className="p-4 mb-6 border border-gray-200 rounded-lg">
                    ...
                    <div className="pt-3 mt-3 border-t border-gray-200">
                      ...
                      {((salad.additionalNutrients?.length ?? 0) > 0) ? (
                        salad.additionalNutrients?.map((nutrient: NutrientInfo, index: number) => (
